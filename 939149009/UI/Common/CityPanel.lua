...
--	CONSTANTS
-- ===========================================================================
local DarkenLightenColor = UI.DarkenLightenColor
...
function ViewMain( data:table )
	...
	local toPlusMinus = function(value)
	...
	if data.FoodSurplus >= 0 then
		...
		table.insert(tToolTip, Locale.Lookup(
			...
			toPlusMinus(data.FoodSurplus - data.FoodPerTurn)
		))
		table.insert(tToolTip, Locale.Lookup(
			"LOC_SUK_CITY_YIELD_FROM_HAPPINESS_TOOLTIP",
			toPlusMinus(iGrowthModifier * 100 - 100),
			toPlusMinus(iModifiedFood_A - data.FoodSurplus)
		))
		table.insert(tToolTip, Locale.Lookup(
			"LOC_SUK_CITY_YIELD_FROM_HOUSING_TOOLTIP",
			toPlusMinus(data.HousingMultiplier * 100 - 100),
		...
		LuaEvents.SetSuk_YieldTooltip(Controls.FoodGrid, data.YieldFilters[YieldTypes.FOOD], toPlusMinus(iModifiedFood_H), sFoodTT)
	else
		Controls.FoodCheck:GetTextButton():SetText(			"[ICON_Food]"		..toPlusMinusString(data.FoodSurplus))
		...
		local sFoodTT = table.concat(tToolTip, "[NEWLINE]");
		RealizeYield3WayCheck( data.YieldFilters[YieldTypes.FOOD], YieldTypes.FOOD, sFoodTT)
		LuaEvents.SetSuk_YieldTooltip(Controls.FoodGrid, data.YieldFilters[YieldTypes.FOOD], toPlusMinus(data.FoodSurplus), sFoodTT)		
	end
	-----------------------------------
	-----------------------------------
...
function OnCityMadePurchase(owner:number, cityID:number, plotX:number, plotY:number, purchaseType, objectType)
	if m_GrowthPlot ~= -1 then
		local growthPlotX:number, growthPlotY:number = Map.GetPlotLocation(m_GrowthPlot);
